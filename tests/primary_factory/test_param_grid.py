...
generate_param_grid = _param_grid_module.generate_param_grid


def test_generate_param_grid():
    """Test parameter grid generation with constraint filtering."""
    ...
    # 过滤后: 2 rates × (5<20, 5<30, 10<20, 10<30) × 2 vertical = 2×4×2 = 16
    # Note: 10<20 is valid (10 is less than 20)
    assert len(grid) == 16
    # Verify fast < slow constraint
    assert all(grid['fast'] < grid['slow'])
    # Verify combo_id uniqueness
    assert grid['combo_id'].nunique() == 16
...
def test_generate_param_grid_columns():
    ...
    expected_columns = ['combo_id', 'cusum_rate', 'fast', 'slow', 'vertical_bars']
...
def test_generate_param_grid_empty():
    ...
    assert len(grid) == 0
    assert list(grid.columns) == ['combo_id', 'cusum_rate', 'fast', 'slow', 'vertical_bars']


def test_generate_param_grid_sorting():
    """Test that results are sorted by rate, fast, slow, vertical_bars."""
    grid = generate_param_grid(
        cusum_rates=[0.15, 0.05],  # Unsorted input
        fast_windows=[15, 5],  # Unsorted input
        slow_windows=[30, 20],  # Unsorted input
        vertical_bars=[30, 10],  # Unsorted input
    )
