# API Reference: gnpr_distance.py

**Language**: Python

**Source**: `codependence/gnpr_distance.py`

---

## Functions

### spearmans_rho(x: np.array, y: np.array) → float
...
### gpr_distance(x: np.array, y: np.array, theta: float) → float
...
### gnpr_distance(x: np.array, y: np.array, theta: float, bandwidth: float = 0.01) → float
