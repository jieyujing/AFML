...
See Also:
    ...
    - :class:`afmlkit.bar.data_model.TradesData`: Creates the HDF5 files that this class inspects.
    - :meth:`afmlkit.bar.data_model.TradesData.save_h5`: Method that creates the HDF5 structure.
    - :meth:`afmlkit.bar.data_model.TradesData.load_trades_h5`: Complementary loading functionality.
...
##### get_metadata(self, key: str) → Dict[str, any]
...
##### get_integrity_info(self, key: str) → Optional[pd.DataFrame]
...
##### get_statistics(self, key: str) → Dict[str, any]
...
##### inspect_gaps(self, max_gap: pd.Timedelta = pd.Timedelta(minutes=1), processes: int = 4) → Dict[str, list[tuple[pd.Timestamp, pd.Timedelta]]]
...
|------|------|---------|-------------|
| self | None | - | - |
| max_gap | pd.Timedelta | pd.Timedelta(minutes=1) | - |
| processes | int | 4 | - |

**Returns**: `Dict[str, list[tuple[pd.Timestamp, pd.Timedelta]]]`


##### get_integrity_summary(self, verbose = True) → Dict[str, Dict]
...
.. important::
    This class assumes the source HDF5 file follows the structure created by :class:`afmlkit.bar.data_model.TradesData`.
...
.. note::
    ...
    :class:`afmlkit.bar.kit.TimeBarKit` directly to build bars from trades data.
...
See Also:
    ...
    :class:`afmlkit.bar.data_model.TradesData`: Creates the source HDF5 files with trades data.
    :meth:`afmlkit.bar.data_model.TradesData.save_h5`: Saves trades data to HDF5 format which is used by this class to add time bars to it.
    :class:`afmlkit.bar.kit.TimeBarKit`: The underlying time bar construction engine.
    :class:`afmlkit.bar.base.BarBuilderBase`: Base class for bar construction strategies.
...
##### __init__(self, h5_path: str, keys: list[str] = None)
...
##### _check_keys(self, keys: list[str])
...
##### process_key(self, key: str, overwrite: bool = False) → bool
...
##### process_all(self, overwrite: bool = False) → Dict[str, bool]
...
.. important::
    ...
    The input H5 file must be generated with :class:`afmlkit.bar.data_model.TradesData` and :class:`AddTimeBarH5`.
...
Examples:
   ...
   >>> # doctest: +SKIP
   >>> from afmlkit.bar.io import TimeBarReader
...
See Also:
    ...
    - :class:`afmlkit.bar.data_model.TradesData`: Underlying trades data structure for the bar construction process.
    - :class:`afmlkit.bar.kit.TimeBarKit`: Time bar construction engine used by the processing pipeline.
...
##### _list_time_range(self) → Tuple[pd.Timestamp, pd.Timestamp]
...
**Returns**: `Tuple[pd.Timestamp, pd.Timestamp]`


##### _find_relevant_keys(self, start_time: Optional[pd.Timestamp] = None, end_time: Optional[pd.Timestamp] = None) → List[str]
...
##### read(self, start_time: Optional[Union[str, pd.Timestamp, dt.datetime]] = None, end_time: Optional[Union[str, pd.Timestamp, dt.datetime]] = None, timeframe: Optional[str] = None) → pd.DataFrame
...
##### _resample(self, df: pd.DataFrame, timeframe: str) → pd.DataFrame
...
### _find_gaps(key: str, filepath: str, max_gap: pd.Timedelta) → Tuple[str, list[Tuple[pd.Timestamp, pd.Timedelta]]]
...
**Returns**: `Tuple[str, list[Tuple[pd.Timestamp, pd.Timedelta]]]`



### w_median(sub: pd.DataFrame) → float
