...
### 1. Meta-Labeling (元标签) - 优先级: 高 🔥
- **目标**: 训练二级模型 (Secondary Model) 过滤一级模型的假阳性信号。
- **依据**: AFML Chapter 3。虽然一级模型能预测涨跌，但准确率可能仅略高于随机。Meta-labeling 专注预测"一级模型是否正确"，从而提高策略的 Sharpe Ratio。
- **行动**: 实现 `MetaLabeling` 类，使用一级模型的预测作为特征之一。
