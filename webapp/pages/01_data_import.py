    ...
    if prepared_data is None:
        ...
        if bar_type == 'time':
            ...
            if st.button("构建 CUSUM K 线"):
                st.info("CUSUM 过滤 K 线功能")

                try:
                    from afmlkit.sampling import CUSUMFilter

                    cusum_filter = CUSUMFilter(threshold=threshold)
                    filter_events = cusum_filter.filter(prepared_data['close'])

                    # 在过滤事件处采样
                    bar_data = prepared_data.loc[filter_events].copy()

                    SessionManager.update('bar_data', bar_data)
                    SessionManager.update('bar_config', {'type': bar_type, 'threshold': threshold})
                    SessionManager.update('filter_events', filter_events)

                    st.success(f"检测到 {len(filter_events)} 个 CUSUM 事件")
                    st.dataframe(bar_data.head(10))

                except ImportError:
                    st.warning("CUSUM Filter 尚未实现")
