...
def compute_labels_and_weights(
    ...
    [⚠️ 危险 / WARNING]
    由于 Trend Scan 已经重构为"前瞻性 (Forward-looking)"的纯标签生成算法，它包含了未来信息。
    在真实应用的 Meta-Labeling 中，将其 `side` 注入特征必定导致极度严重的前视偏差 (Look-ahead Bias)。
    此处的代码集成纯粹为了演示 API 流水线结构，实修策略时，Primary Signal 必须替换为无偏的动量或预测模型。
    ...
    trend_df : pd.DataFrame, optional
        来自 trend_scan_labels() 的输出 DataFrame。若提供则注入 side 并计算权重。
    """
    ...
    # 确保 side 和 t_value 也被包含进去（如果存在）
    if is_meta and trend_df is not None:
        aligned_trend = trend_df.loc[result_df.index]
        final_df['side'] = aligned_trend['side']
        final_df['t_value'] = aligned_trend['t_value']
...
def main():
    ...
    # Step 2: Trend Scan Labeling — 前瞻性动态趋势标签 (含有未来信息！)
    ...
    print("\n" + "=" * 60)
    print("Running Forward Trend Scan Labeling...")
    print("WARNING: This method uses future prices. Demo purpose only for Meta-Labeling integration.")
    print("=" * 60)
    ...
    if 'side' in trend_df.columns:
        ...
        if pd.api.types.is_datetime64_any_dtype(trend_df['t1']) and len(trend_df) > 0:
            median_duration = (trend_df['t1'] - trend_df.index).median()
            print(f"  Median duration : {median_duration}")
    
    # Step 3: Meta-Labeling — Trend Scan Side + t_value 驱动三重屏障
    # ⚠️ 此处接入前瞻性的 Trend Scan 仅作代码集成 API 的演示，因为此处的 side 使用了未来数据。
