...
def load_data(path: Path) -> tuple[pd.DataFrame, pd.Series, pd.Series, pd.Series]:
    ...
    # ── Robust Cleaning ──────────────────────────────────────────────
    # 1. Drop any feature columns that are entirely NaN
    nan_cols = X.columns[X.isna().all()]
    if len(nan_cols) > 0:
        print(f"[Data] Dropping all-NaN features: {nan_cols.tolist()}")
        X = X.drop(columns=nan_cols)

    # 2. Forward-fill then backward-fill remaining NaN
    X = X.ffill().bfill()

    # 3. Drop features that are constant (zero variance)
    # Correlation will be NaN for constant columns, breaking distance matrix
    const_cols = X.columns[X.nunique() <= 1]
    if len(const_cols) > 0:
        print(f"[Data] Dropping constant features: {const_cols.tolist()}")
        X = X.drop(columns=const_cols)

    # 4. Final check for any remaining non-finite values (Inf/-Inf)
    X = X.replace([np.inf, -np.inf], np.nan).ffill().bfill()
    
    # If still NaN exists (entire column was Inf etc), drop it
    still_nan = X.columns[X.isna().any()]
    if len(still_nan) > 0:
        print(f"[Data] Dropping features with persistent NaN/Inf: {still_nan.tolist()}")
        X = X.drop(columns=still_nan)

    print(f"[Data] Total features remaining: {X.shape[1]}")
