...
**Source**: `backtest_statistics/statistics.py`

---

## Functions

### timing_of_flattening_and_flips(target_positions: pd.Series) → pd.DatetimeIndex
...
### average_holding_period(target_positions: pd.Series) → float
...
### bets_concentration(returns: pd.Series) → float
...
### all_bets_concentration(returns: pd.Series, frequency: str = 'M') → tuple
...
### drawdown_and_time_under_water(returns: pd.Series, dollars: bool = False) → tuple
...
### sharpe_ratio(returns: pd.Series, entries_per_year: int = 252, risk_free_rate: float = 0) → float
...
### information_ratio(returns: pd.Series, benchmark: float = 0, entries_per_year: int = 252) → float
