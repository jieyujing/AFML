# API Reference: sadf.py

**Language**: Python

**Source**: `structural_breaks/sadf.py`

---

## Functions

### _get_sadf_at_t(X: pd.DataFrame, y: pd.DataFrame, min_length: int, model: str, phi: float) → float
...
### _get_y_x(series: pd.Series, model: str, lags: Union[int, list], add_const: bool) → Tuple[pd.DataFrame, pd.DataFrame]
...
### _lag_df(df: pd.DataFrame, lags: Union[int, list]) → pd.DataFrame
...
### get_betas(X: pd.DataFrame, y: pd.DataFrame) → Tuple[np.array, np.array]
...
### _sadf_outer_loop(X: pd.DataFrame, y: pd.DataFrame, min_length: int, model: str, phi: float, molecule: list) → pd.Series
...
### get_sadf(series: pd.Series, model: str, lags: Union[int, list], min_length: int, add_const: bool = False, phi: float = 0, num_threads: int = 8, verbose: bool = True) → pd.Series
