"""
10_combined_backtest.py - AL9999 DMA + Meta 组合策略回测

流程:
1. 加载 DMA 信号 + Meta Model 预测
2. Filter-First 阈值优化（guard-aware rolling backtest）
3. 三层退出网络：DMA 反向交叉 + 波动率 SL/TP + 时间耗尽
4. 动态仓位管理：meta 概率 -> 仓位大小
5. DSR 验证、可视化

AFML 规范:
- 考虑参数试验次数对 DSR 的影响
- Meta Filter 仅使用 OOF/OOS 预测，无前视偏差
"""
...
def load_honest_meta_signals(models_dir: str, precision_threshold: float) -> tuple[pd.DataFrame, pd.Timestamp, str]:
    ...
    :returns: (signals_df, holdout_start, scheme)
    """
...
def main():
    logger.info("=" * 60)
    logger.info("  AL9999 DMA + Meta Combined Backtest")
    logger.info("=" * 60)
    ...
    if not os.path.exists(dma_path):
        raise FileNotFoundError(f"缺少 DMA 信号文件: {dma_path}。请先运行 04_dma_primary_model.py。")
    ...
    # 加载 Dollar Bars (提前加载以计算 touch_idx)
    bars = pd.read_parquet(_resolve_bars_path())

    # 2. 合并 DMA 信号与 Meta 预测
    ...
    # FIX Critical 2: 处理 NaN 波动率目标（用前向填充，再用全局均值填充剩余 NaN）
    vol_targets_raw = events_features.loc[common_idx_for_tbm, TBM_CONFIG['target_ret_col']]
    vol_targets = vol_targets_raw.ffill().fillna(vol_targets_raw.mean()).values.astype(np.float64)

    # 2.5.3 准备 triple_barrier 输入
    timestamps = bars.index.values.astype(np.int64)
    close = bars['close'].values.astype(np.float64)

    # FIX Critical 1: 过滤掉不在 bars.index 中的事件，避免 KeyError
    valid_bar_mask = pd.Series(common_idx_for_tbm).isin(bars.index)
    n_not_in_bars = (~valid_bar_mask).sum()
    if n_not_in_bars > 0:
        logger.warning(f"  跳过 {n_not_in_bars} 个不在 bars.index 中的事件")
    combined_tbm = combined_tbm[valid_bar_mask.values]
    common_idx_for_tbm = common_idx_for_tbm[valid_bar_mask.values]
    vol_targets = vol_targets[valid_bar_mask.values]
    sides = combined_tbm['side'].values.astype(np.int8)
    event_idxs = np.array([
        bars.index.get_loc(ts) for ts in common_idx_for_tbm
    ], dtype=np.int64)

    # FIX Critical 2 (cont.): 过滤掉剩余 NaN 的波动率目标
    nan_mask = ~np.isnan(vol_targets)
    ...
    # Primary (仅 DMA，无 Meta 过滤)
    primary_signals = combined.copy()
    primary_signals['meta_pred'] = 1  # 不过滤
    ...
    price_bh = bars['close'] - bars['close'].iloc[0]
    ...
    if combined_oos_perf['dsr'] > 0.95:
        logger.info("[PASS] 策略具备实盘潜力 (DSR > 95%%)")
    else:
        logger.info("[WARN] 策略存在过拟合风险 (DSR <= 95%%)")
