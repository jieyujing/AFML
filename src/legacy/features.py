...
class FracDiffFeatureGenerator:
    ...
    def generate(
        self, df: pd.DataFrame, content_cols: List[str] = ["close", "volume"]
    ) -> pd.DataFrame:
        """
        ...
        for col in content_cols:
            ...
            for w in self.windows:
                ...
                features[f"{base_feature_name}_SLOPE{w}"] = self._rolling_slope(
                    ffd_series, w
                )
...
class Alpha158FeatureGenerator:
    ...
    def _rolling_features(self, df: pd.DataFrame, window: int) -> pd.DataFrame:
        ...
        features[f"IMIN{w}"] = l.rolling(w).apply(
...
def main():
    """Generate comprehensive features for labeled dollar bars using OO approach."""
    print("=" * 80)
    print("Feature Engineering: FeatureEngineer (Alpha158 + FFD + Market Regime)")
    print("=" * 80)
    ...
    # 2. Using OO FeatureEngineer
    print("\n2. Using FeatureEngineer...")
    engineer = FeatureEngineer(
        windows=[5, 10, 20, 30, 50], ffd_check_stationarity=True, ffd_d=0.4
    )
    ...
    print("\n3. Combining features with labels...")
    ...
    if engineer.d_values_:
        print("\nOptimized FFD Parameters (Memory Preservation):")
        for col, d in engineer.d_values_.items():
            w = engineer._get_weights_ffd(d, 1e-4, len(df))
            print(
                f"  {col.upper():<10} | d = {d:.2f} | Memory Window: {len(w):>4} bars"
            )
    ...
    for label, count in label_counts.items():
        print(
            f"  Class {label:>2}: {count:>6} samples ({count / total_samples * 100:>5.1f}%)"
        )
