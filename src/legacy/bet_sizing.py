...
def main():
    """Demonstrate bet sizing using OO approach."""
    print("=" * 80)
    print("Bet Sizing - AFML Chapter 10")
    print("=" * 80)
    ...
    # Create example probabilities and predictions
    prob_series = pd.Series(0.6 + np.random.randn(len(df)) * 0.1, index=df.index)
    prob_series = prob_series.clip(0.01, 0.99)

    # Calculate bet sizes
    bet_sizes = sizer.calculate(
        events=df, prob_series=prob_series, pred_series=None, average_active=True
    )

    print(f"   Calculated {len(bet_sizes)} bet sizes")
    print(f"   Mean bet size: {bet_sizes.mean():.4f}")
    print(f"   Max bet size: {bet_sizes.max():.4f}")

    # 3. Save results
    print("\n3. Saving results...")
    df["bet_size"] = bet_sizes
    df.to_csv("bet_sizes.csv")
    print("   ✓ Saved to bet_sizes.csv")

    print("\n" + "=" * 80)
    print("✓ Bet Sizing Complete!")
    print("=" * 80)
