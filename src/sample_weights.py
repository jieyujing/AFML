...
def num_concurrent_events(close_series_index, label_endtime, molecule):
    ...
    idx_start = nearest_index[0] if isinstance(nearest_index, (list, np.ndarray)) else nearest_index
    idx_end = nearest_index[1] if isinstance(nearest_index, (list, np.ndarray)) and len(nearest_index) > 1 else -1 
    ...
    relevant_close = close_series_index[(close_series_index >= start_time) & (close_series_index <= end_time)]
...
def main():
    ...
    # 2. Calculate Average Uniqueness
    print("\n2. Calculating Average Uniqueness...")
    print("   (This measures how much information a label shares with others)")
    
    # num_threads=1 for safety and simplicity
    avg_uniqueness = get_av_uniqueness_from_triple_barrier(
    ...
    print("   (Weights based on absolute returns and uniqueness)")
    ...
    output_df = pd.DataFrame(index=events.index)
    output_df['avg_uniqueness'] = avg_uniqueness['tW']
    output_df['sample_weight'] = weights
